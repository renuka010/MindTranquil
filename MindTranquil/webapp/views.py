...
def update_session_api(request):
    ...
    end_time = datetime.datetime.strptime(
        request.data.get("end_time"), "%Y-%m-%dT%H:%M:%S.%fZ"
    )
    if serializer.is_valid() and not (end_time.hour == 0 and end_time.minute == 0):
        ...
        if (not user_stats.last_session_date) or (
            current_date - user_stats.last_session_date > timedelta(days=1)
        ):
...
def get_active_days_api(request):
    ...
    sessions = Sessions.objects.filter(
        username=request.user, end_time__year=year, end_time__month=month
    )
    ...
    data = {"active_days": list(active_days), "sessions": sessions_serializer.data}
